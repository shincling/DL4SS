...
class Attention(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        if self.mode == 'dot':
            ...
            # # -> (batch, time, spec_dim, align_hidden) dot (align_hidden, 1)
            # # -> (batch, time, spec_dim, 1)
            # energy = K.dot(mix_embed_l, self.v_align)
            # # -> (batch, time, spec_dim)
            # energy = K.reshape(energy, (-1, self.time_step, self.spec_dim))
        else:
...
class SpkLifeLongMemory(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        spk_vector_eps = K.switch(K.equal(spk_vector_l, 0.), np.spacing(1), spk_vector_l)  # 防止分母为0
        ...
        life_long_mem_eps = K.switch(K.equal(life_long_mem, 0.), np.spacing(1), life_long_mem)  # 防止分母为0
...
class SelectSpkMemory(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        if K.dtype(target_spk_l) != 'int32':
            target_spk_l = K.cast(target_spk_l, 'int32')
        # (None(batch), spk_size, embed_dim)，长时记忆单元
...
def update_memory(auditory_model, target_spk, spk_memory):
    ...
    # 根据目标说话人idx和spk_memory计算新的memory
