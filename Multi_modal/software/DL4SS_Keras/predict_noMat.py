...
def eval_separation(model, audio_list, valid_test, epoch_num, log_file, spk_to_idx, batch_size=1, spk_num=2
    ...
    for line_idx, line in enumerate(file_list):
        ...
        if unk_spk:
            ...
            batch_clean_wav.append(tar_supp_sig[:supp_time * config.FRAME_RATE])
        ...
        if (batch_count == batch_size) or (line_idx == (file_list_len-1)):
            ...
            batch_size = len(batch_input_mix_fea)
            ...
            target_input_spk = np.array(batch_input_spk).reshape((batch_size, 1))
            ...
            for _pred_output in list(target_pred):
                ...
                mix_wav = batch_mix_wav[batch_idx].tolist()
                target_wav = batch_target_wav[batch_idx].tolist()
                noise_wav = batch_noise_wav[batch_idx].tolist()
                pred_wav = _pred_wav.tolist()
                if epoch_num == 0:
                ...
                if (line_idx < batch_size) and (batch_idx == 0):
                    # 把预测写到文件中
