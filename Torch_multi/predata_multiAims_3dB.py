        ...
        if config.DATASET=='WSJ0': #开始构建数据集
            ...
            while True:
                ...
                if 1 and config.dB and config.MAX_MIX==3 and mix_k==3:
                    'DB with 3 channels.'
                    dB_rate_large=10**(config.dB/20.0*(0.5+0.5*np.random.rand()))#10**(0.125——0.25)
                    dB_rate_small=10**(config.dB/20.0*(0.5*np.random.rand()))#10**(0--0.125)
                    dB_rate_normal=10**(config.dB/20.0*0.5)#10**(0--0.125)
                    channel_act='ALL'
                    # rrr=np.random.rand()
